// VENDOR.ME Cheat Engine for ESP32 - ULTIMATE EDITION
...
// =================================================================================================
// WEB PAGE (HTML, CSS, JS) - FINAL VERSION
// =================================================================================================
...
<head>
    ...
    <style>
        :root {
            ...
            --container-bg: rgba(26, 26, 52, 0.8);
        ...
        body::before {
            background-image: radial-gradient(ellipse 50px 50px at 20% 20%, rgba(255,0,255,0.2) 0%, transparent 100%), radial-gradient(ellipse 50px 50px at 10% 80%, rgba(255,0,255,0.2) 0%, transparent 100%);
            animation: move-boxes-1 25s linear infinite;
        }
        body::after {
            background-image: radial-gradient(ellipse 50px 50px at 80% 80%, rgba(255, 165, 0, 0.2) 0%, transparent 100%), radial-gradient(ellipse 50px 50px at 90% 10%, rgba(255, 165, 0, 0.2) 0%, transparent 100%);
            animation: move-boxes-2 25s linear infinite;
        }
        body { background-color: var(--background-color); color: var(--text-color); font-family: var(--font-family); margin: 0; padding: 0; overflow: hidden; font-size: 18px; }
        ...
        #menu-page h2, #app-page h2 { font-size: 32px; color: var(--neon-glow); text-shadow: 0 0 10px var(--neon-glow); margin-bottom: 30px; }
        ...
        #cheat-area h2 { margin-top: 0; flex-shrink: 0; text-align: center; padding-bottom: 15px; }
        ...
        #status-window h2 { margin-bottom: 40px; }
...
<body>
    <div id="login-page" class="page">
        <div class="login-box"><h1>VEND.ME</h1><input type="text" id="username-input" class="login-input" placeholder="Username"><input type="password" class="login-input" placeholder="Password"><button id="login-button" class="login-button">Authenticate</button></div>
    </div>
    ...
    <div id="app-page" class="page hidden">
        <div id="app-header"><h1>VEND.ME</h1></div>
        <button id="back-to-menu-btn" class="nav-button">&larr; Menu</button>
        <button id="logout-btn" class="nav-button">Logout</button>
        <button id="mute-btn" class="nav-button">🔊</button>
        <div id="app-main"><div id="cheat-area"></div></div>
    </div>
    <div id="status-window" class="modal-overlay hidden">
        ...
        <button class="nav-button" style="position:relative; top: 40px;" data-modal-close="status-window">&larr; Back to Menu</button>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
          try {
            ...
            const gameData = {
                "Warzone": {"Toggles": [{ name: "Player ESP"},{ name: "No Recoil"}], "Sliders": [{ name: "Aimbot FOV", min: 1, max: 100 }]},
                "BO6": {"Toggles": [{ name: "Silent Aim"},{ name: "Box ESP"}], "Sliders": [{ name: "Aim Assist", min: 0, max: 100 }]},
                "R6 Siege": {"Toggles": [{ name: "No Recoil"},{ name: "Gadget ESP"}], "Sliders": [{ name: "Speed %", min: 100, max: 130 }]},
                "Fortnite": {"Toggles": [{ name: "Player ESP"},{ name: "No Bloom"}], "Sliders": [{ name: "Aimbot Strength", min: 0, max: 100}]},
                "GTA V": {"Toggles": [{ name: "God Mode"},{ name: "Never Wanted"}], "Sliders": [{ name: "RP Multiplier", min: 1, max: 100}]},
                "Apex Legends": {"Toggles": [{ name: "Glow ESP"},{ name: "No Recoil"}], "Sliders": [{ name: "Aim Smoothing", min: 0, max: 100}]}
            };
            ...
            function playSound(url) { try { const src = audioContext.createBufferSource(); fetch(url).then(r=>r.arrayBuffer()).then(d=>audioContext.decodeAudioData(d)).then(b=>{src.buffer=b; src.connect(audioContext.destination); src.start(0);}); } catch(e){} }
            ...
            function buildMainMenu() {
                ...
                Object.keys(gameData).forEach(name => {
                    const card = document.createElement('div'); card.className = 'menu-card'; card.textContent = name;
                    card.addEventListener('click', () => { playSound(clickSound); loadGameUI(name); showPage('app'); });
                    elements.menuGrid.appendChild(card);
                });
                const statusCard = document.createElement('div'); statusCard.className = 'menu-card'; statusCard.textContent = 'System Status';
                statusCard.addEventListener('click', () => { playSound(clickSound); modals.status.classList.remove('hidden'); });
                elements.menuGrid.appendChild(statusCard);
            }
            function loadGameUI(name) { elements.cheatArea.innerHTML = ''; const h2=document.createElement('h2'); h2.textContent=`${name} Cheats`; elements.cheatArea.appendChild(h2); const tabs=document.createElement('div'); tabs.className='cheat-tabs'; const tT=document.createElement('button'); tT.className='tab-button active'; tT.textContent='Toggles'; const sT=document.createElement('button'); sT.className='tab-button'; sT.textContent='Sliders'; tabs.appendChild(tT); tabs.appendChild(sT); const tC=document.createElement('div'); tC.className='tab-content'; const sC=document.createElement('div'); sC.className='tab-content hidden'; (gameData[name]?.Toggles||[]).forEach(c=>{tC.appendChild(createCheatItem(c));}); (gameData[name]?.Sliders||[]).forEach(c=>{sC.appendChild(createCheatItem(c));}); tT.addEventListener('click',()=>{tT.classList.add('active');sT.classList.remove('active');tC.classList.remove('hidden');sC.classList.add('hidden');}); sT.addEventListener('click',()=>{sT.classList.add('active');tT.classList.remove('active');sC.classList.remove('hidden');tC.classList.add('hidden');}); elements.cheatArea.appendChild(tabs); elements.cheatArea.appendChild(tC); elements.cheatArea.appendChild(sC); }
            ...
            document.body.innerHTML = `<h1>Critical Error</h1><p>${e.message}</p>`;
          }
        });
    </script>
</body>
