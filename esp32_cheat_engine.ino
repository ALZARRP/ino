...
// Reverted to "Cyberpunk Edition" and applied final layout fixes as requested.
...
<head>
    ...
    <style>
        ...
        #menu-page h2 { font-size: 28px; color: var(--neon-glow); text-shadow: 0 0 10px var(--neon-glow); margin-bottom: 30px; }
        ...
        #status-window h2 { font-size: 28px; color: var(--neon-glow); text-shadow: 0 0 10px var(--neon-glow); margin-bottom: 30px; }
...
<body>
    ...
    <div id="app-page" class="page hidden"><div id="app-header"><h1>VEND.ME</h1></div><button id="back-to-menu-btn" class="nav-button">&larr; Menu</button><button id="logout-btn" class="nav-button">Logout</button><div id="app-main"><div id="cheat-area"></div></div></div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
          try {
            ...
            const elements={usernameInput:document.getElementById('username-input'),loginButton:document.getElementById('login-button'),enterButton:document.getElementById('enter-button'),menuGrid:document.getElementById('menu-grid'),cheatArea:document.getElementById('cheat-area'),backToMenuButton:document.getElementById('back-to-menu-btn'),logoutButton:document.getElementById('logout-btn'),backgroundMusic:document.getElementById('bg-music')};
            const gameData = {"Warzone":{"Toggles":[{name:"Player ESP"},{name:"No Recoil"}], "Sliders":[{name:"Aimbot FOV",min:1,max:100}]},"BO6":{"Toggles":[{name:"Silent Aim"},{name:"Box ESP"}], "Sliders":[{name:"Aim Assist",min:0,max:100}]},"R6 Siege":{"Toggles":[{name:"No Recoil"},{name:"Gadget ESP"}], "Sliders":[{name:"Speed %",min:100,max:130}]},"Fortnite":{"Toggles":[{name:"Player ESP"},{name:"No Bloom"}], "Sliders":[{name:"Aimbot Strength",min:0,max:100}]},"GTA V":{"Toggles":[{name:"God Mode"},{name:"Never Wanted"}], "Sliders":[{name:"RP Multiplier",min:1,max:100}]},"Apex Legends":{"Toggles":[{name:"Glow ESP"},{name:"No Recoil"}], "Sliders":[{name:"Aim Smoothing",min:0,max:100}]}};
            ...
            function showAlert(msg,type){playSound(notificationSound);const el=document.createElement('div');el.className=`alert ${type}`;el.textContent=msg;document.body.appendChild(el);setTimeout(()=>el.classList.add('show'),10);setTimeout(()=>{el.classList.remove('show');setTimeout(()=>document.body.removeChild(el),500);},2000);}
