// VENDOR.ME Cheat Engine for ESP32 - ENHANCED WITH NFC, SOUNDS & VISUALS
// Enhanced with NFC Login, Button Sounds, and Improved Visual Effects
...
// =================================================================================================
// To use NFC login, you need to write a URL to your NFC tag.
// 1. On your phone, use an NFC writing app (e.g., "NFC Tools").
// 2. Write a new "URL / URI" record to your tag.
// 3. The URL should be: http://192.168.4.1/#nfc_login_token=YOUR_SECRET_TOKEN_HERE
//
// The default IP address for the ESP32 access point is 192.168.4.1.
// The default secret token is defined below as `validNfcToken`. You can change it for security.
//
// When you tap the tag, your phone will open this URL and log you in automatically.
// =================================================================================================
...
String currentUser = "";
const String validNfcToken = "vendor-me-secret-key-12345"; // CHANGE THIS FOR BETTER SECURITY
...
// =================================================================================================
// WEB PAGE (HTML, CSS, JS) - ENHANCED WITH NFC, SOUND AND IMPROVED VISUALS
// =================================================================================================
...
<body>
    ...
    <div id="login-page" class="page">
        ...
        <div class="login-box">
            <div style="text-align: center; padding: 20px 0;">
                <svg width="80" height="80" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" style="color: var(--neon-primary); animation: pulse 2s infinite ease-in-out;">
                    <path d="M2.92,7.28A9.3,9.3,0,0,1,12,2.5a9.3,9.3,0,0,1,9.08,4.78"/>
                    <path d="M18.84,10.35a5.4,5.4,0,0,1,0,3.3"/>
                    <path d="M5.16,10.35a5.4,5.4,0,0,0,0,3.3"/>
                    <path d="M15.19,12.5a3.18,3.18,0,0,1-6.36,0"/>
                    <path d="M21.08,16.72A9.3,9.3,0,0,1,12,21.5a9.3,9.3,0,0,1-9.08-4.78"/>
                </svg>
                <h2 style="color: var(--neon-secondary); margin-top: 20px; font-size: 24px;">Tap to Authenticate</h2>
                <p style="color: #aaa; font-size: 16px; line-height: 1.5; margin: 10px 0 0;">
                    Hold your VENDOR.ME keycard near the top of your phone to log in.
                </p>
            </div>
    ...
    <script>
        ...
        async function handleNfcLogin() {
            if (window.location.hash.startsWith('#nfc_login_token=')) {
                const token = window.location.hash.substring('#nfc_login_token='.length);

                // Clean up the URL
                history.pushState("", document.title, window.location.pathname + window.location.search);

                const messageDiv = document.getElementById('login-message');
                messageDiv.innerHTML = '<div class="info-message">NFC token detected. Authenticating...</div>';

                try {
                    const response = await fetch('/nfc-login', {
                        method: 'POST',
                    ...
                    if (result.success) {
                        messageDiv.innerHTML = '<div class="success-message">Authentication successful!</div>';
                        showNotification('Welcome to VENDOR.ME!', 'success');
                        setTimeout(() => {
                            showPage('menu-page');
                        }, 1000);
                    } else {
                        messageDiv.innerHTML = '<div class="error-message">Invalid NFC Token.</div>';
...
void handleNfcLogin(AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total) {
  String jsonString = "";
  for (size_t i = 0; i < len; i++) {
    jsonString += (char)data[i];
  }

  // Parse JSON manually for token
  int tokenStart = jsonString.indexOf("\"token\":\"") + 9;
  int tokenEnd = jsonString.indexOf("\"", tokenStart);
  String token = jsonString.substring(tokenStart, tokenEnd);

  String response = "";
  if (token == validNfcToken) {
    userLoggedIn = true;
    currentUser = "admin"; // Set a default username
    ...
    tft.setTextSize(3);
    tft.drawString("NFC LOGIN", 40, 60);
    ...
    tft.setTextSize(1);
    tft.drawString("User: " + currentUser, 60, 160);
    ...
    response = "{\"success\":false,\"message\":\"Invalid NFC Token\"}";
    ...
    tft.setTextSize(1);
    tft.drawString("Invalid token", 60, 160);
...
void setup() {
  ...
  tft.setTextColor(TFT_YELLOW);
  tft.setTextSize(1);
  tft.drawString("NFC Enhanced Edition", 50, 130);
  tft.setTextColor(TFT_WHITE);
  ...
  server.on("/nfc-login", HTTP_POST, [](AsyncWebServerRequest *request) {}, NULL, handleNfcLogin);
...
void loop() {
  ...
  if (currentMillis - lastUpdate >= 3000) { // Update every 3 seconds
    ...
    if (!userLoggedIn) {
      ...
      tft.setTextColor(TFT_WHITE);
      tft.setTextSize(1);
      tft.drawString("Enhanced with NFC & Sound", 30, 90);
